...
Contributing: see [`CONTRIBUTING.md`](./CONTRIBUTING.md). Published on npm as **`bwai-cli`** (`npx bwai-cli`; after install use **`bwai-cli`** or alias **`bwai`**). Landing: https://boilerplates-with-ai-skills.vercel.app
...
node dist/cli.js export-plugin nextjs-app /tmp/bwai.nextjs-app
...
- `src/` — `bwai` CLI (`list-boilerplates`, `doctor`, `list-workflows`, `new`, `export-plugin`, `scan-catalog`, `scan-project`, `search-skills`, `promote`, `sync-skills`, `sync-upstream`)
